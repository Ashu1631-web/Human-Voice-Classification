...
.metric-card .lbl { font-size: 0.78rem; color: #aaa; margin-top: 4px; letter-spacing: 0.06em; }
...
.login-card {
    ...
    box-shadow: 0 0 60px rgba(0,240,208,0.12);
}
.login-title {
    font-family: 'Orbitron', sans-serif;
...
.pred-male   { background: rgba(255,58,58,0.18); border: 2px solid var(--red);  color: var(--red);  }
.pred-female { background: rgba(255,79,163,0.18); border: 2px solid var(--pink); color: var(--pink); }
...
.conf-fill-f { height: 100%; background: linear-gradient(90deg, #ff4fa3, #ff85c8); border-radius: 8px; transition: width 0.6s; }
.conf-fill-m { height: 100%; background: linear-gradient(90deg, #ff3a3a, #ff7a00); border-radius: 8px; transition: width 0.6s; }
...
def page_eda(df):
    ...
    with c1:
        fig = px.scatter(df, x="mean_pitch", y="rms_energy", color="label",
                         color_discrete_map={"Female": FEMALE_COLOR, "Male": MALE_COLOR},
                         opacity=0.7,
                         title="🔵 Pitch vs RMS Energy (by Gender)")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
...
def page_classification(df):
    ...
    with c2:
        ...
        fig = px.bar(means[top_features].T.reset_index().melt(id_vars="index"),
                     ...
                     color_discrete_map={"Female": FEMALE_COLOR, "Male": MALE_COLOR},
                     barmode="group",
                     title="2️⃣ Mean Feature Values by Gender (top 8 diff)")
    ...
    with c2:
        fig = px.box(df, x="label", y="mean_spectral_centroid", color="label",
                     color_discrete_map={"Female": FEMALE_COLOR, "Male": MALE_COLOR},
                     title="4️⃣ Spectral Centroid Distribution by Gender")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
    ...
    with c2:
        fig = px.violin(df, x="label", y="zero_crossing_rate", color="label",
                        color_discrete_map={"Female": FEMALE_COLOR, "Male": MALE_COLOR},
                        box=True,
                        title="6️⃣ Zero Crossing Rate (Violin) by Gender")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
    ...
    with c2:
        fig = px.scatter(df, x="mean_pitch", y="std_pitch", color="label",
                         color_discrete_map={"Female": FEMALE_COLOR, "Male": MALE_COLOR},
                         opacity=0.65,
                         title="8️⃣ Mean Pitch vs Pitch Std Dev")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
    ...
    with c1:
        ...
        fig = px.pie(pie_data, names="Gender", values="Count",
                     color="Gender",
                     color_discrete_map={"Female": FEMALE_COLOR, "Male": MALE_COLOR},
                     title="9️⃣ Class Balance Pie Chart",
                     hole=0.4)
        st.plotly_chart(apply_theme(fig), use_container_width=True)
    ...
    with c2:
        fig = px.box(df, x="label", y="mean_spectral_rolloff", color="label",
                     color_discrete_map={"Female": FEMALE_COLOR, "Male": MALE_COLOR},
                     title="🔟 Spectral Rolloff by Gender")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
...
def page_clustering(df):
    ...
    with col2:
        fig = px.scatter(df, x="mean_spectral_centroid", y="mean_spectral_bandwidth",
                         ...
                         color_discrete_sequence=[FEMALE_COLOR, MALE_COLOR],
                         opacity=0.7,
                         title="Centroid vs Bandwidth (Clusters)")
        st.plotly_chart(apply_theme(fig), use_container_width=True)
